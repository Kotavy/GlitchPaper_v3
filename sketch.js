...
function setup() {
  ...
  createCanvas(960,540); // Half of 1920 x 1080 ratio for testing, set blockSize var to 30 to emulate full HD at scale
  fill(0);
...
// For whatever reason mousePressed() only wants to work in the main file.
...
function draw() {
  ...
  // If the user clicks, change the color around that location based on current variance
  if (mouseIsPressed) {
    localizedColors();
  }
  ...
  text("Area of effect is: " + affect, 20, 40)
  text("Speed: " + xspeed + "x, " + yspeed + "y.", 20, 60);
  text("Press 's' to save as glitch_wp", 20, 80);
}
